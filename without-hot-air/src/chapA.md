...
Two of this appendix's figures cannot be carried forward, and the reason is worth stating rather than leaving them silently unmentioned. Figure A.12 sets measured fuel consumption against speed for a Prius and a BMW 318ti: the Prius source in note 5 now returns a 404, and the BMW page is still up but publishes its numbers as plots rather than as a table, so this edition cannot re-read them at the resolution the figure needs. What that figure asserts — that real consumption does not vary as the square of speed — is not in doubt, and the rolling and stop-start terms are why. Figure A.13, powers of cars against their top speeds, is Tennekes' scatter rather than a measurement of MacKay's, and redrawing it would mean assembling a new sample of cars from specifications this edition has no citable source for, so it stands as printed. Its claim, that power goes as the cube of speed, is the claim figure A.9a draws three times.[^steady]
