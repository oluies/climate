...
**Neither has reached the tractor, and the tractor's duty cycle is not the truck's.** A lorry runs continuously and needs energy where it happens to be; a tractor returns to the same yard every night and does most of its heaviest work in a few weeks of the year. That argues for charging slowly on the farm rather than for either a motorway swap station or a megawatt cable — except in exactly the weeks when it matters, when the machine runs sixteen hours and the slow charge is no use at all. **The seasonal peak is the whole difficulty**, and it is the reason the liquid keeps winning: a fuel tank is the cheapest energy store anyone has ever built, and a farm needs its largest store for about a fortnight a year.
