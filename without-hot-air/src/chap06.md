...
Longer life and less energy-intensive manufacture do push the energy return upward, but they are not what separates this chapter's 4 from chapter M's **10 or more**. Most of that gap is a change of accounting boundary rather than of hardware: chapter M harmonises every fuel to point of use, which charges the thermal sources for the two-thirds they lose in a power station and leaves electricity-producing sources largely untouched. The reordering follows from the method, and it would have happened with 2008 panels.

Two of chapter M's cautions cut the other way and belong here rather than in a footnote. The first is **PROI**: a fast-growing industry scores far below its own facility ratio, because each year's output is repaying the energy sunk into building the next year's capacity — onshore wind's facility ratio of about 70 becomes roughly 25 at the industry level, and solar is growing faster than wind. At 512 GW a year, the fleet's return is meaningfully worse than any single panel's. The second is that adding storage and overbuild to make an intermittent source firm has diminishing returns, which is exactly what the last section of this chapter is about.
