...
**That is not a criticism of the plan so much as a description of what an intensity target is**, and China has never disguised the point: the binding commitment is the peak, and the peak is a date rather than a number. What this chapter can add is the arithmetic that turns one into the other — and the observation that on the electricity figures above, China has been adding non-fossil generation fast enough that the peak is being discussed as something that may already have happened rather than as something scheduled.
