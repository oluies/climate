...
**A few of them brake regeneratively**, which the section on cars above discusses at length and which most e-bikes cannot do at all. The reason is mechanical rather than electrical: a mid-drive motor delivers through the chain and a geared hub motor sits behind a freewheel, so in both cases the wheel turns without turning the motor and there is nothing to generate with. Only a direct-drive hub motor is rigidly coupled to the wheel. Stromer, which uses one, offers two braking strengths — RECUP 1 for light descents and RECUP 2 for steep ones — and switches the recovery off when the battery is full, since a full battery has nowhere to put it.[^ebikeregen]

The arithmetic says where this is worth having. A rider and machine of about 100 kg descending 100 metres give up *mgh* = 98 kJ, or 0.027 kWh. At the 0.6 kWh per 100 km of the table above that is four and a half kilometres of riding, and after the losses of turning motion into electricity and back it is perhaps two or three. Stopping from 25 km/h is worth far less: the kinetic energy is 2.4 kJ, about a hundred metres of range, and half of that after losses. So a bicycle inverts MacKay's finding for cars. He notes that regenerative braking pays most in *city* driving, where a heavy vehicle is repeatedly accelerated and stopped. A bicycle is too light for its stops to be worth much and its rider is not carrying enough battery to care, but a long descent is a long descent whatever the vehicle weighs. On the flat it is a rounding error. In the hills it is real, which is presumably why the manufacturer that fits it is Swiss.
...
[^ebikeregen]: The RECUP 1 and RECUP 2 modes, their use on light and steep descents, and the fact that recovery is disabled when the battery is full are from Stromer's own support documentation, <https://www.stromerbike.com/en/support/faq/battery/what-do-the-regenerative-braking-modes-recup-1-and-recup-2-mean-for-the>. **Stromer publishes no figure for how much energy the modes actually return**, so the numbers in the text are this book's own arithmetic and not the manufacturer's claim: 100 kg of rider and machine descending 100 m gives mgh = 100 x 9.81 x 100 = 98 kJ = 0.027 kWh, and the same mass stopping from 25 km/h (6.9 m/s) gives half mv squared = 2.4 kJ. Both are gross figures for the energy available; the fraction that reaches the battery depends on motor and controller efficiency in generating mode and then on charging losses, and the "two or three kilometres" in the text assumes something between a half and two thirds survives the round trip, which is the range the car literature above reports for electrical regeneration and is used here for want of a bicycle-specific measurement. The 100 kg is a round figure: a Stromer ST model is around 30 kg, heavy for a bicycle because of the battery and the direct-drive hub. The claim that mid-drive and geared-hub systems cannot regenerate is a statement about their transmission rather than about any particular product — a freewheel by construction lets the wheel overrun the motor — and it applies to the Bosch, Shimano and Brose mid-drives named above, which is most of the European market. Note also that a bicycle already has a form of regeneration a car does not: the rider, who can be pedalling on the descent instead of braking, and who on a light machine is the larger part of the system's mass.
